...
python_configure_all() {
	# the wheel build runs its own cmake which ignores mycmakeargs and
	# autodetects CUDA (breaking with gcc>14); pass the toggles explicitly
	DISTUTILS_ARGS=(
		--set DLIB_USE_CUDA=$(usex cuda ON OFF)
		--set DLIB_USE_BLAS=$(usex cblas ON OFF)
		--set DLIB_USE_LAPACK=$(usex lapack ON OFF)
		--set DLIB_NO_GUI_SUPPORT=$(usex X OFF ON)
	)
}
